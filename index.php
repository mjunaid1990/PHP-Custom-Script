...
if(isset($_GET['open_message_id'])) {
    @setcookie('open_message_id', $_GET['open_message_id'], time() + 31556926, '/');
}

if(isset($_GET['open_notifications'])) {
    @setcookie('open_notifications', 1, time() + 31556926, '/');
}

if($url_action == 'pro' && IS_LOGGED === true) {
    if(!isset($_COOKIE['countdown-'.$active_user->id])) {
        $curr_date = date('Y-m-d');
        if($config->countdown_hours && $config->coupon_repeat_date == $curr_date) {
            $end_date = date("Y-m-d H:i:s", strtotime('+'.$config->countdown_hours.' hours'));
            setcookie('countdown-'.$active_user->id, $end_date, time()+86400);
        }
    }
}
...
if($is_admin) {
    if($active_user->admin != 1) {
        if($active_user && $_SERVER['REQUEST_METHOD'] != 'POST') {
            if( !empty($active_user->filter_profile_setting_words) && $url_action != 'suspensionverification') {
                header("Location: ".$config->uri.'/suspension-verification');
                exit();
            }
            if( !empty($active_user->filter_general_setting_words) && $url_action != 'suspensionverification') {
                header("Location: ".$config->uri.'/suspension-verification');
                exit();
            }
        }
...
//    stop user from editing other users profile
//    added by Community Dev LLC
    $not_allowed = false;
    if(isset($active_user->admin) && $active_user->admin == 0) {
        switch ($url_action) {
            case 'settings':
                $route2 = route(2);
                if(!empty($route2)) {
                    if($route2 != $active_user->username) {
                        $not_allowed = true;
                    }
                }
                break;
...
if($url_action == 'personalityquestions' && IS_LOGGED === false) {
    header("Location: ./");
    exit();
}
...
//                exit();
//            }
            //new step by Community Dev LLC
            $contoller_userverify_file = $_CONTROLLERS . 'personalityquestions.php';
            if (file_exists($contoller_userverify_file)) {
                if (!class_exists('PersonalityQuestions', false)) {
                    require_once $contoller_userverify_file;
                }
                PersonalityQuestions::show();
                exit();
            }
        }
    }
