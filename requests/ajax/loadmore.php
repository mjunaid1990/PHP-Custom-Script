    ...
    function match_users($mode='findmatches') {
        ...
        if ($error == '') {
            ...
            foreach ($match_users as $key => $value) {
                ...
                $user->body = isset($value['body']) && (!empty($value['body']) ? Dataset::load('body')[$value['body']] : $value['body']);//$value['body'];
                $user->about = isset($value['about']) && (!empty($value['about']) ? $value['about'] : '');//$value['body'];
                $user->smoke = isset($value['smoke']) && (!empty($value['smoke']) ? Dataset::load('smoke')[$value['smoke']] : $value['smoke']);//$value['smoke'];
                $user->ethnicity = isset($value['ethnicity']) && (!empty($value['ethnicity']) ? Dataset::load('ethnicity')[$value['ethnicity']] : $value['ethnicity']);//$value['ethnicity'];
                $user->pets = isset($value['pets']) && (!empty($value['pets']) ? Dataset::load('pets')[$value['pets']] : $value['pets']);//$value['pets'];
