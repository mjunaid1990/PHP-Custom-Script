...
Class Profile extends Aj {
    ...
    public function approve_story(){
        ...
        if( self::ActiveUser()->id == $story_to_userid ) {
            $story = $db->where('id', $storyid)->getOne('success_stories', array('quote','slug'));
            ...
            if ($saved) {
                ...
                if ($Notification) {
                    $Notification->createNotification(self::ActiveUser()->web_device_id, self::ActiveUser()->id, $storyuserid, 'approve_story', '', '/story/' . $story['slug']);
                }
                return array(
                    'status' => 200,
                    'message' => __('Story approved successfully.'),
                    'url' => self::Config()->uri . '/story/' . $story['quote']
                );
            } else {
                return array(
                    'status' => 400,
    ...
    public function disapprove_story(){
        ...
        if( self::ActiveUser()->id == $story_to_userid ) {
            ...
            if ($saved) {
                $Notification = LoadEndPointResource('Notifications');
                if ($Notification) {
                    $Notification->createNotification(self::ActiveUser()->web_device_id, self::ActiveUser()->id, $storyuserid, 'disapprove_story', '', '/story/'.$story['slug']);
                }
                return array(
                    'status' => 200,
...
//        modified by Community Devs LLC
        $slug_ = url_slug($quote,array('delimiter' => '-',));

        $slugs = $db->where('quote', $quote)->get('success_stories');
        if(!empty($slugs) && count($slugs) > 0) {
            $slug_num = count($slugs);
        }
        $slug = isset($slug_num)?$slug_.'-'.$slug_num:$slug_;
//        end added by Community Devs LLC
        ...
        if( $is_user_exist['id'] !== null && $is_user_exist['id'] > 0 ){
            ...
            $saved = $db->insert('success_stories',array(
                ...
                'status' => 0,
                'slug' => $slug, //this is new field added by muhammad junaid
                'created_at' => time()
            ));
            if($saved) {
                $Notification = LoadEndPointResource('Notifications');
    ...
    public function set_profile_avatar() {
        global $db, $_UPLOAD, $_DS;
        if (self::ActiveUser() == NULL) {
            return array(
                'status' => 403,
        ...
        if ($error == '') {
            ...
            return array(
                'status' => 200,
                'message' => __('File set as profile avatar successfully'),
            );
...
//            }
    ...
    //save step form added by Community Devs LLC
    public function save_step_form() {
        global $db;
        if (self::ActiveUser() == NULL) {
            return array(
                'status' => 403,
        ...
        if(isset($data['activities_after_marriage'])) {
            $data['activities_after_marriage'] = implode(',',$data['activities_after_marriage']);
        }
        if(isset($data['activities_feeling_stressed'])) {
            $data['activities_feeling_stressed'] = implode(',',$data['activities_feeling_stressed']);
        }
        if(isset($data['qualities'])) {
            $data['qualities'] = implode(',',$data['qualities']);
//            $data['character'] = implode(',',$data['qualities']);
        }
...
//            start
            ...
            if (!empty($_POST['custom_fields'])) {
                ...
                $fields2 = GetProfileFields('profile');
                foreach ($fields2 as $key => $field2) {
                    $name2 = $field2['fid'];
            ...
            if (!empty($field_data)) {
                UpdateUserCustomData(self::ActiveUser()->id, $field_data);
            }
//            end
            $_SESSION[ 'userEdited' ] = true;
            return array(
    ...
    //save step form added by Community Devs LLC
    public function save_step_form_single() {
        global $db;
        if (self::ActiveUser() == NULL) {
            return array(
                'status' => 403,
        ...
        if($data['activities_after_marriage']) {
            $data['activities_after_marriage'] = implode(',',$data['activities_after_marriage']);
        }
        if($data['activities_feeling_stressed']) {
            $data['activities_feeling_stressed'] = implode(',',$data['activities_feeling_stressed']);
        }
        if($data['qualities']) {
            $data['qualities'] = implode(',',$data['qualities']);
        ...
        if(!empty(self::ActiveUser()->facebook)) {
            $data['verified'] = '1';
            $data['status'] = '1';
            $data['active'] = '1';
        }
        if(!empty(self::ActiveUser()->twitter)) {
            $data['verified'] = '1';
            $data['status'] = '1';
            $data['active'] = '1';
        }
...
//            firstname and lastname validation hided by Community Devs LLC
...
//            end by Community Devs LLC
            ...
            if ($error == '') {
                ...
                if (isset($_POST[ 'city' ])) {
                  ...
                  //added location to save by Community Devs LLC
                //                added incomplete address, country, state and city to save by muhammad junaid
                
                if (isset($_POST[ 'location' ])) {
                    $user[ 'location' ] = Secure($_POST[ 'location' ]);
                }
                if (isset($_POST[ 'incomplete_address' ])) {
                    $user[ 'incomplete_address' ] = Secure($_POST[ 'incomplete_address' ]);
                }
                if (isset($_POST[ 'country_id' ])) {
                    $user[ 'country_id' ] = Secure($_POST[ 'country_id' ]);
                }
                if (isset($_POST[ 'state_id' ])) {
                    $user[ 'state_id' ] = Secure($_POST[ 'state_id' ]);
                }
                if (isset($_POST[ 'city_id' ])) {
                    $user[ 'city_id' ] = Secure($_POST[ 'city_id' ]);
                }
                
                //modified by Community Devs LLC
                if(isset($_POST[ 'is_location_verified' ]) && !empty($_POST[ 'is_location_verified' ])) {
                    $user[ 'is_location_verified' ] = Secure($_POST[ 'is_location_verified' ]);
                }else {
                    $user[ 'is_location_verified' ] = 0;
                }
                
                if (isset($_POST[ 'filter_general_setting_words' ])) {
                    $user[ 'filter_general_setting_words' ] = Secure($_POST[ 'filter_general_setting_words' ]);
                }
                
                if (isset($_POST[ 'filter_profile_setting_words' ])) {
                    $user[ 'filter_profile_setting_words' ] = Secure($_POST[ 'filter_profile_setting_words' ]);
                }
                // end by Community Devs LLC
...
//                    suspended account email trigger
//                    added by Community Devs LLC
                    
                    $suspended = false;
                    if (isset($user[ 'filter_general_setting_words' ]) && !empty($user[ 'filter_general_setting_words' ]) ) {
                        $suspended = true;
                    }
                    if($suspended) {
                        //to member
                        SendEmailCustom(self::ActiveUser()->email, 4);
                    }
    ...
    public function save_profile_setting() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if (isset($_POST[ 'character' ])) {
                if(is_array($_POST['character'])) {
                    $user[ 'character' ] = implode(',', $_POST['character']);
                }else {
            ...
            if (!empty($user)) {
                $saved = $db->where('id', $target_id)->update('users', $user);
                if ($saved) {
//                    send email if unwanted activity found
//                    added by Community Devs LLC
                    $usr = $db->where('id', $target_id)->getOne('users');
                    if (isset($user[ 'filter_profile_setting_words' ]) && !empty($user[ 'filter_profile_setting_words' ])) {
                        $suspended = true;
                    }
                    if(isset($suspended)) {
                        //to memeber
                    ...
                    if (!empty($_POST['custom_fields'])) {
                        ...
                        foreach ($fields as $key => $field) {
                            ...
                            if (isset($_POST[$name])) {
                                ...
                                $field_data[] = array(
                                    $name => $name == 'fid_44'? nl2br($_POST[$name]):$_POST[$name] // modify by Community Devs LLC
                                );
                            }
                        }
                    }
                    ...
                    $prof = userProfile(self::ActiveUser()->id);
                    return array(
                        'status' => 200,
                        'message' => '<p> ' . __('Profile data saved successfully.') . '</p>',
                        'completion'=>$prof->profile_completion, //added by Community Devs LLC
                        'show_popup'=>is_profile_complete_popup_display(self::ActiveUser()->id), //added by Community Devs LLC
                        'balance'=>self::ActiveUser()->balance
                    );
                } else {
                ...
                return array(
                    'status' => 200,
                    'message' => '<p> ' . __('Profile data saved successfully.') . '</p>',
                    'completion'=>self::ActiveUser()->profile_completion, //added by Community Devs LLC
                    'show_popup'=>is_profile_complete_popup_display(self::ActiveUser()->id), //added by Community Devs LLC
                    'balance'=>self::ActiveUser()->balance
                );
            }
    ...
    //added by Community Devs LLC
    public function profile_complete_reward() {
        global $db;
        $db->where('user_id',self::ActiveUser()->id)->update('profile_completion',array('is_message_displayed' => (int)$_POST['is_message_displayed']));
        return array(
                'status' => 200,
            );
    }
