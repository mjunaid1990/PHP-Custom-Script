...
Class Chat extends Aj {
    ...
    function open_private_conversation() {
        ...
        if ($_user) {
            ...
            return array(
                ...
                'is_chat_initiated'=>get_new_chat_initate_detail($user_from->id), // modified by Community Devs LLC
                'to' => array(
                    'id' => $user_from->id,
    ...
    function get_conversation_list($accepted = 1) {
        ...
        $_user  = LoadEndPointResource('users'); //added by Community Devs LLC
        ...
        foreach ($conversations as $key => $msg) {
            ...
            $convers = $db->objectBuilder()->where('id', $msg->id)->get('conversations');

            
            $u1 = $_user->get_user_profile($convers[0]->sender_id);
            $u2 = $_user->get_user_profile($convers[0]->receiver_id);

            if( $u1->is_pro == 0 && $u2->is_pro == 0 ) {
                $textLength = strlen($msg->text);
                $msg->text  = mb_substr($msg->text, 0, 3,'utf-8').str_repeat('*', $textLength - 1);
            }
