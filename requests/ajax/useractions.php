...
//                            $url                   = $config->uri . '/steps';
                            $url = $config->uri . '/personality-questions'; //modified by Community Devs LLC
...
//                            $url = $config->uri . '/steps'; // old step stopped by Community Devs LLC
                            $url = $config->uri . '/personality-questions'; // added by Community Devs LLC
                        }
...
//    added by Community Devs LLC
    function get_email_verification_code_popup() {
        global $db;
        if (self::ActiveUser() == NULL) {
            return array(
                'status' => 403,
        ...
        $email = $db->where('email', Secure($_GET['email']))->where('is_verified', 0)->where('user_id', self::ActiveUser()->id)->getOne('verify_popup_email',array('*'));
        if ($email && $email['code'] !== '') {
            $db->where('id', $email['id'])->update('verify_popup_email',array('is_verified'=>1));
            return array(
                'status' => 200,
                'code' => $email['code']
            );
...
//            balance column added by Community Devs LLC
            $user = $db->where('username', $userid)->getOne('users',array('id','web_device_id','is_pro','balance'));
            if(empty($user)){
                return array(
                    'status' => 400,
...
//                updated credit increment based on like

            // changes by Community Devs LLC

                if($user) {
                    $like_points = self::Config()->cost_per_like;
                    $add_credit = floatval($user['balance']) + floatval($like_points);
    ...
    function remove_like() {
        ...
        if (isset($_POST[ 'username' ]) && !empty($_POST[ 'username' ])) {
            $userid = Secure($_POST[ 'username' ]);
            $user = $db->where('username', $userid)->getOne('users',array('id', 'balance'));
            if(empty($user)){
                return array(
                    'status' => 400,
                    'message' => __('The User not exist.')
            ...
            if (isset($_POST[ 'userid' ]) && !empty($_POST[ 'userid' ])) {
                ...
                $user = $db->where('id', $userid)->getOne('users',array('id','balance'));
                if(empty($user)){
                    return array(
                        'status' => 400,
                        'message' => __('The User not exist.')
        ...
        //  updated credit decrease based on unlike

        // changes by Community Devs LLC

        if($user) {
            $same_user_like = $db->where('user_id', self::ActiveUser()->id)->where('like_userid', $user['id'])->where('is_like', '1')->getOne('likes');
            if($same_user_like) {
                $unlike_points = self::Config()->cost_per_like;
                ...
                if($balance == 1.00) {
                    ...
                    $dec_credit = $balance - floatval($unlike_points);
                    $db->where('id', $user['id'])->update('users', array(
                        'balance' => $dec_credit
                    ));
                }
