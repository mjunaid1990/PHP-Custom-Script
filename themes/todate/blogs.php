		...
		<div class="valign-wrapper qd_blog_sub_hdr">
			...
			<div class="qd_blog_cats_list">
				...
				<ul id="blog_cat_dropdown" class="dropdown-content">
					<li><a href="<?php echo $site_url;?>/blogs" data-ajax="/blogs"><?php echo __('ALL');?></a></li>
					<?php
						$blog_categories = Dataset::blog_categories();
						foreach ($blog_categories as $key => $category) {
					?>
						<li><a href="<?php echo $site_url;?>/blogs/<?php echo $key . '_' . url_slug(html_entity_decode($category));?>" data-ajax="/blog/<?php echo $key . '_' . url_slug(html_entity_decode($category));?>"><?php echo $category;?></a></li>
					<?php }?>
				</ul>
			</div>
		</div>
