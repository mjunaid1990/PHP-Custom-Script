    ...
    <div class="row r_margin">
			...
			<div class="to_user_media">
				<div class="avatar">
					<a class="inline" href="<?php echo $profile->avater->full;?>" id="avater_profile_img">
						<img src="<?php echo $profile->avater->full;?>" alt="<?php echo $profile->first_name;?>" class="responsive-img" />
					</a>
					<?php if( $user !== null ){ ?>
						<?php if( $user->admin == 1 ){ ?>
				...
				<div class="dt_user_pro_info">
					...
					<?php } ?>
                                        ...
                                        <!--new chat initiate popup for confirmation updated by Community Devs LLC-->
        ...
        <div class="col s12 l7">
			<?php if ($q['private_count'] > 0 && !$matched) { ?>
				<div class="alert alert-warning"><?php echo(str_replace('{X}', $profile->first_name, __('you_have_to_match_with_media'))); ?></div>
			<?php } ?>
		
			<div class="dt_user_profile dt_user_info">
				<div class="info">
					<div class="combo dt_othr_ur_info">
                        <h1 style="font-size: 30px; margin: 0; font-weight: 500;"><?php echo $profile->first_name.$profile->pro_icon;?><?php echo ( $profile->age  > 0 ) ? ", ". $profile->age : "";?>
...
//                                            profile header add by Community Devs LLC
    ...
    font-size: 15px; margin-bottom: 9px;">'.$profile_header.'</h5>';
                                            }
                                            ?>
				...
				<?php if( !empty( $profile->location ) ) {?>
					...
					<div class="to_usr_info_block">
						<h5><?php echo __( 'Location' );?></h5>
                                                <!--location is replaced to incomplete address by Muhammad Junaid-->
						<?php
...
//                                                                                if avatar exist then show otherwise show full image
        ...
        //                                                                                    by Community Devs LLC
				...
				<div class="to_usr_info_block to_usr_info_things">
					...
					<div class="dt_profile_info">
						...
						<?php } ?>
                                                        <!--added and modify by Community Devs LLC-->
						<?php
...
//                                                                                            by muhammad junaid
					...
					<div class="dt_profile_info <?php echo $display == false?'hide':''; ?>">
						...
						<?php if( !empty( $profile->children ) ) {?>
							<div class="row">
								<div class="col s6 m5"><p class="info_title"><?php echo __( 'Have Children?' );?></p></div>
								...
								foreach ($general_fields as $key => $field) {
									...
									if($field['profile_page'] == 1) {
										if( isset($field['fid']) && isset($general_custom_data[$field['fid']]) && $general_custom_data[$field['fid']] !== null ) {
											$hide = 'hide';
                                                                                        if ($field['select_type'] == 'yes') {
                                                                                            if(isset($field['fid']) && $field['fid'] != 'fid_11') {
                                                                                                $options = @explode(',', $field['type']);
                                                                                                array_unshift($options,"");
                                                                                                unset($options[0]);
                                                                                            }
					...
					<div class="dt_profile_info <?php echo $display == false?'hide':''; ?>">
						...
						<?php } ?>
                                        ...
                                        if (count($general_fields) > 0) {
                                                foreach ($general_fields as $key => $field) {
                                                        if($field['profile_page'] == 1) {
                                                                if( isset($field['fid']) &&  isset($general_custom_data[$field['fid']]) && !empty($general_custom_data[$field['fid']]) ) {
                                                                    $display = true;
                                                                }
                                                        }
                                                }
											...
											echo '<div class="col s6 m5"><p class="info_title">' . $field['name'] . '</p></div>';
												...
												if (isset($options[$general_custom_data[$field['fid']]])) {
													...
													echo '<div class="col s6 m7"><p>' . $general_custom_data[$field['fid']] . '</p></div>';
                                                                                            ...
                                                                                            if(isset($field['fid']) && $field['fid'] == 'fid_44') {
                                                                                                echo '<div class="col s6 m7"><p class="'.$read_more_cls.'" data-controller="'.$d_contr.'" style="white-space: pre-wrap;">' . breaktonewline($general_custom_data[$field['fid']]).'</p>'.$read_more.'</div>';
                                                                                            }else {
                                                                                                echo '<div class="col s6 m7"><p>' . $general_custom_data[$field['fid']] . '</p></div>';
                                                                                            }
...
//    }
	...
	if(isset($user->id) && ( Wo_IsFollowRequested( $profile->id, (int) $user->id  ) ) ){
    ?>
        <div id="story_approval" class="modal">
            <div class="modal-content">
                ...
                <h6 class="bold center"><?php echo $profile->first_name . ' ' . __('request your friendship.');?></h6>
                <p class="center"></p>
            </div>
            <div class="modal-footer">
