...
<div class="container page-margin">
	<div class="qd_read_blog_thumb">
		<a class="inline" href="<?php echo GetMedia($data['article']['thumbnail']);?>">
			<img src="<?php echo GetMedia($data['article']['thumbnail']);?>" title="<?php echo $data['article']['img_alt']?$data['article']['img_alt']:$data['article']['title'];?>" alt="<?php echo $data['article']['img_alt']?$data['article']['img_alt']:$data['article']['title'];?>" />
		</a>
	</div>
	<div class="row r_margin qd_read_blog_row">
		...
		<div class="col l10">
			<div class="dt_sections qd_read_blog_section">
				<div class="qd_read_blog_head">
					...
					<div class="valign-wrapper">
						<a href="<?php echo $site_url;?>/blogs/<?php echo $data['article']['category'] . '_' . url_slug(html_entity_decode(Dataset::blog_categories()[$data['article']['category']]));?>" data-ajax="/blogs/<?php echo $data['article']['category'] . '_' . url_slug(html_entity_decode(Dataset::blog_categories()[$data['article']['category']]));?>"><?php echo Dataset::blog_categories()[$data['article']['category']];?></a>
						<span class="middot">·</span>
						<time><?php echo get_time_ago($data['article']['created_at']) ;?></time>
						<span class="middot">·</span>
                                                <!--modified view count by Community Devs LLC-->
						<span><?php echo (int)$data['article']['view'] == 0?(5000 * 1):(5000 + (int)$data['article']['view']);?> <?php echo __('Views');?></span>
			...
			<ul class="qd_read_blog_tags">
				...
				?>
					<li><a href="<?php echo $site_url;?>/blogs/<?php echo trim($tag);?>" data-ajax="/blogs/<?php echo trim($tag);?>"><?php echo $tag;?></a></li>
				<?php }?>
			</ul>
		</div>
		<div class="col l1"></div>
