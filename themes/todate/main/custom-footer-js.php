...
<!--change email model added by Community Devs LLC-->
<div id="change-user-email" class="modal change-user-email">
    <div class="modal-content" style="text-align: center;">
            <h4><?php echo __('Change Email');?></h4>
            <div class="modal-body verify-email-content">
...
<?php if(isset(auth()->admin) && auth()->admin == '1') { ?>
<!--change username model added by Community Devs LLC-->
<div id="change-username-modal" class="modal">
    <div class="modal-content" style="text-align: center;">
            <h4><?php echo __('Change Username');?></h4>
            <div class="modal-body verify-username-content">
...
<script>
    ...
    function avatarimage(img, $user) {
        var src = $(img).parent().parent().find('.fancybox-image').attr('src'); // Looks for the *fancyboxed* image
        var url = window.ajax + 'profile/set_profile_avatar';
        var formData = new FormData();
        formData.append("id", $(img).attr('data-id') );
    ...
    function custom_footer_js(page_name){
        <?php if( isset( $_SESSION['JWT'] ) ){ ?>
            if( window.current_page === "myprofile" ){
                //added new button to select any image as avatar
                //by  Community Devs LLC
            $.fancybox.defaults.btnTpl.avatar = '<button data-fancybox-avatar class="fancybox-button fancybox-button--avatar" title="Set as profile picture"  onClick="javascript:avatarimage(this, <?php echo $profile->id;?>)" >' +
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"><path d="M20 22H4v-2a5 5 0 0 1 5-5h6a5 5 0 0 1 5 5v2zm-8-9a6 6 0 1 1 0-12 6 6 0 0 1 0 12z" fill="currentColor"></path></svg>'  +
                '</button>';
        //end by Community Devs LLC
            ...
            $.fancybox.defaults.btnTpl.remove_prvt = '<button data-fancybox-remove-prvt class="fancybox-button fancybox-button--remove-prvt" title="Make it public"  onClick="javascript:unprivateimage(this)" >' +
                ...
                //added new button to select any image as avatar
                //by  Community Devs LLC
                $.fancybox.defaults.btnTpl.avatar = '';
                //end by Community Devs LLC
        ...
        $('[data-fancybox="gallery"]').fancybox({
            beforeShow: function(){
                ...
                $('.fancybox-button--avatar').attr('data-id', this.opts.id); // added by Community Devs LLC
            ...
            buttons : [
                ...
                'remove_prvt',
                'del',
                'avatar',//added by Community Devs LLC
                'zoom',
                'slideShow',
                'thumbs',
                'close'
       ...
       //
        ...
        $("form").submit(function(e) {
            ...
            var button_save = form.find('.btn-settings-save span').text();  
            var button_save_next = form.find('.btn-save-and-next span').text();
            var button_save_back = form.find('.btn-save-and-back span').text();
            var button_save_finish = form.find('.btn-save-and-finish span').text();
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    form.find('.btn-settings-save span').text( button_save );
                    form.find('.btn-save-and-next span').text( button_save_next );
                    form.find('.btn-save-and-back span').text( button_save_back );
                    form.find('.btn-save-and-finish span').text( button_save_finish );
...
//                    reload page if prohibited activity is used on profile
//added Community Devs LLC
                    if(form.attr('action') === '/profile/save_general_setting'){
                        var sv = $('input[name="filter_general_setting_words"]').val();
                        if(sv) {
                            window.location.reload();
                        }
                    }else if(form.attr('action') === '/profile/save_profile_setting') {
                        var sv = $('input[name="filter_profile_setting_words"]').val();
                        if(sv) {
                            window.location.reload();
                        }
                    }
