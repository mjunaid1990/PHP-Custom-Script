...
<head>
    ...
    <?php// } ?>
    <input type="hidden" name="fl" value="<?php echo $config->max_photo_per_user?$config->max_photo_per_user:10; ?>" />
    <input type="hidden" name="characters_length" value="<?php echo characters_length(); ?>" />
    <input type="hidden" name="pf" value="<?php echo $config->cost_per_profile_completion?(int)$config->cost_per_profile_completion:0; ?>" />
    <input type="hidden" name="ch" value="<?php echo countdown_seconds(); ?>" />
    <?php if($config->is_countdown_active == 1 && $config->coupon_discount) { ?>
    <input type="hidden" name="cop" value="<?php echo $config->coupon_discount; ?>" />
    <?php } ?>
    <?php
    $open_message_id = '';
    $open_noti_id = '';
    if (isset($_COOKIE['open_message_id'])) {
        $open_message_id = $_COOKIE['open_message_id'];
    }
    if (isset($_COOKIE['open_notifications'])) {
        $open_noti_id = 1;
    }
    ?>
    <input type="hidden" name="msid" value="<?php echo $open_message_id; ?>" />
    <input type="hidden" name="noid" value="<?php echo $open_noti_id; ?>" />
    <?php if(IS_LOGGED === true) { ?>
    <input type="hidden" name="aut" value="<?php echo auth()->id; ?>" />
    <?php } ?>
