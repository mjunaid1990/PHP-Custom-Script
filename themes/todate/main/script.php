<?php
    foreach ($data['jsfiles'] as $key => $js_file) {
        echo '<script src="'. $theme_url . $js_file . '?v='.time().'" type="text/javascript" id="' . $key . '"/></script>';
    }
    if( isset( $_SESSION['JWT'] ) ){
        require_once ( $theme_path . 'main' . $_DS . 'timeago.php' );
        echo '<script src="'. $theme_url . 'assets/js/chat.js?v='.time().'" type="text/javascript" id="chat-script"/></script>';
