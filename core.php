    ...
    if ( isset( $_COOKIE['JWT'] ) && !isset($_GET['language']) ) {
        ...
        if(isset($_COOKIE['activeLang']) && !empty($dafult_user_lang)){
            if( $dafult_user_lang['language'] !== $_COOKIE['activeLang'] ){
                if( strpos($_COOKIE['activeLang'], ',') !== false ) {
                    return 'english';
                }
        ...
        if(!empty($dafult_user_lang) && $dafult_user_lang['language'] !== $lang ){
            ...
            if( strpos($dafult_user_lang['language'], ',') !== false ) {
                return 'english';
            }
...
//print_r($config->default_language);die();
...
//    create a separate layout for suspension verification page
//    added by Community Devs LLC
    $suspension_base_file = $theme_path . 'base-suspension.php';
    $personality_base_file = $theme_path . 'base-personality.php';
...
//        added this logic by Community Devs LLC
        if($file == 'suspension-verification') {
            require($suspension_base_file);
        }else if($file == 'personality-questions') {
            require($personality_base_file);
        }else if($file == 'upload-photo') {
            require($personality_base_file);
        }else {
            require($base_file);
        }
...
//        'tv'
    ...
    $profile_completion_custom_fields       = array(
        'fid_1', // here to
        'fid_11', // from
        'fid_12', // to
        'fid_27', // you are serious for marriage
        'fid_28', // What I'm looking for in a partner?
        'fid_29', // My promises to my partner
        'fid_30', // Willing to relocate
        'fid_31', // Visa support to your partner
        'fid_13', // occupation
        'fid_16', // Annual Income
        'fid_22', // complexion
        'fid_17', // Weight
        'fid_19', // Hair Type
        'fid_18', // Hair Lenght
        'fid_23', // Eye Color
        'fid_24', // Eye Wear
        'fid_25', // My appearance considered as
        'fid_26', // Physical / Health Status
//        'fid_37', // Number of children
//        'fid_38', // Oldest Child
//        'fid_39', // Youngest Child
        'fid_40', // If Allah wills, would you want to have kids?
        'fid_34', // Do you smoke?
        'fid_35', // Do you drink?
        'fid_36', // Eating Habits
        'fid_33', // Religion
        'fid_32', // Born / Reverted
        'fid_41', // Religious Level
        'fid_42', // Read Quran
        'fid_44', // What I love most about Islam
        'fid_43', // What is your reference as a life curriculum?
        'fid_45', // Polygamy
        'fid_46', // Family Values
        'fid_47', // Profile Creator
        'fid_49', // home type
        'fid_50' // living situation
    );
    
    $profile_completion_fields_count = count($profile_completion_fields) + count($profile_completion_custom_fields);
    ...
    $user_custom_profile_fields_row = GetProfileFieldsById($username);
    ...
    if ($db->count > 0) {
        ...
        //display only user images
        //->where('is_identity',0) added to filter only user images
        //added by Community Devs LLC
        $mediafiles = $db->where('user_id', trim($profile->id))->where('is_identity',0)->where('is_id_verification',0)->orderBy('id', 'desc')->get('mediafiles', null, array('id','file','is_private','private_file','is_video','video_file','is_confirmed','is_approved'));
        if ($mediafiles) {
            $mediafilesid = 0;
        ...
        foreach ($profile_completion_custom_fields as $p_custom_fields) {
            $fid = isset($user_custom_profile_fields_row[$p_custom_fields])?$user_custom_profile_fields_row[$p_custom_fields]:'';
            if(empty($fid)) {
                $profile_completion_missing[] = $p_custom_fields;
            }else {
                $profile_completion_field++;
            }
        }
        ...
        $profile_completion_value = ((100 * $profile_completion_field) / $profile_completion_fields_count);
        $profile->profile_completion = (int)$profile_completion_value - 5;
        $profile->profile_completion_missing = $profile_completion_missing;

        if($profile->avater->avater) {
            $profile->profile_completion = $profile->profile_completion + 5;
        }
        if($profile->profile_completion == 100) {
//            added credit on profile completion
//            by Community Devs LLC
            $rec = $db->where('user_id', trim($profile->id))->get('profile_completion', null, array('id','user_id'));

            if(empty($rec)) {
                $db->insert('profile_completion', array(
                    'user_id' => trim($profile->id),
                ));
                $like_points = (int)$config->cost_per_profile_completion;
                $add_credit = (int)$profile->balance + $like_points;
                $db->where('id', trim($profile->id))->update('users', array(
                    'balance' => $add_credit
                ));
            }
        }
...
function renderTextMessage(&$message,$msg){
    ...
    if( $msg['_cover'] === true ) { 
        if( (int)$msg['from'] === (int)auth()->id ){
            $message .= '<div class="message" data-messageid="'.$msg['id'].'">' . $msg['_message'] . "<div class='cover-message'>{$msg["to_name"]} cannot read this message as you are both FREE members. <br /><a href='/pro'>Upgrade Now so {$msg["to_name"]} can read this message!</a></div>" . '</div>';
        }else{
            $message .= '<div class="message" data-messageid="'.$msg['id'].'">' . "<div class='cover-message'><a href='/pro'>Upgrade now to read {$msg["from_name"]} Message as you are both FREE memebrs</a></div>" . '</div>';
        }
    } else {
            if( (int)$msg['from'] === (int)auth()->id ){
...
/**
 * Cover message for free users
 * by Community Devs LLC
*/
function isCoverMessage($_user, $message) {
    $to     = $_user->get_user_profile($message['to']);
    $from   = $_user->get_user_profile($message['from']);

    if( $to->is_pro !== '0' || $from->is_pro !== '0' ) return false;
    ...
    if( !empty( $chat_messages ) ) {
        ...
        if ($chat_messages) {
            ...
            foreach ($chat_messages as $key => $value) {
                ...
                // added by Community Devs LLC
                $chat_messages[$key]['_cover'] = isCoverMessage($_user, $value);
...
function sendNotificationEmail($notification){
    global $db,$config;
    $send_mail = false;
    $userEmailNotification = $db->where('id', $notification['recipient_id'])->getOne('users',array('id','username','first_name','last_name','email','email_on_profile_view','email_on_profile_like','email_on_get_gift','email_on_got_new_match','email_on_new_message','email_on_purchase_notifications','email_on_chat_request', 'avater'));
...
//        modify by Community Devs LLC
        ...
        $body = Emails::parse('custom-design-notifications', array(
            'full_name' => $u->full_name,
            'username' => $u->username,
            'avater' => $u->avater->avater,
            'contents' => $notification['contents'],
            'url' => $notification['url'],
            'profile_url' => $config->uri . '/@' . $userEmailNotification['username'],
            'notification_url' => $config->uri.'/find-matches?open_notifications=1',
            'profile_pic'=> GetMedia($userEmailNotification['avater'], false),
            'unsub_url'=>$config->uri.'/settings-email/'.$userEmailNotification['username'],
...
function DeleteExpiredProMemebership() {
    ...
    $sql       = mysqli_query($conn, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
        //check stripe is next payment is made
        // by Community Devs LLC
        $payment = Wo_GetUserLastSubscription($fetched_data['id']);
        $is_paid = false;
        if($payment) {
            $is_paid = Wo_isCustomerPaidCurrentMonth($payment['session_id'], $fetched_data['id']);
        }
        $update_data = false;
        ...
        //checked if payment made user will not be updated
        // by Community Devs LLC
        if($is_paid) {
            $update_data = false;
        }
...
//        modified by Community Devs LLC added type
function RegisterAffRevenue($user_id,$amount = 0,$type='credits'){
    global $db,$config;
...
//        modified by Community Devs LLC
        $old_ref_payment = $db->where('referrar_id',$me['referrer'])->where('user_id',$user_id)->getOne('affiliated_system_payments');
        if(empty($old_ref_payment)) {
                $insert = [
                    'user_id'=>$user_id,
                    'referrar_id'=>$me['referrer'],
                    'amount'=>"$total_revinue",
                    'is_paid'=>1,
                    'type'=>$type,
                    'created_at'=>date('Y-m-d h:i:s')
                ];
            $db->insert('affiliated_system_payments', $insert);
...
function watermark_image($target) {
    ...
    try {
        $theme_url = 'themes/' . $config->theme .'/'; //url modified by Community Devs LLC
